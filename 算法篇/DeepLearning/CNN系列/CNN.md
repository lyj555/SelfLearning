...
- 减少训练参数
  ...
  减少需要处理的元素的个数，加速网络的训练，防止过拟合。

  使网络更容易优化，pooling是每个featuremap单独做**降采样**，与基于卷积的降采样相比，**不需要参数**，更容易优化。
...
- 增加感受视野
  ...
  通过让连续池化层，可以让观测窗口越来越大（即窗口覆盖原始输入的比例越来越大），从而引入空间过滤器的层级结构  

> 这恰好对应上面的那一个问题
>
> 卷积比较精细化，这样意味着可能多数信息都是冗余，而池化则在此基础上进行了再加工，提取更加有效的特征。
...
（2）卷积对应有卷积核，池化对应有池化核。卷积核里面有参数，但是**池化核只是一个框架，里面没有参数**。

（3）使用方面都需要，定义其大小（size），步长（stride），padding类型，都是按照某个模式进行信息的扫描
