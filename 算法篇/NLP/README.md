...
- 第一部分 **NLP基础算法**
  ...
  - 学习算法
    ...
    > 大致构想了下算法的几个领域：
    >
    > - Contextualized pretraining network
    >
    >   比如bert，ernie
    >
    > - Trick-relevant algorithm
    >
    >   Imbalance problem, optimization
    >
    > - domain paper
    >
    >   qa，classification，mrc等
...
- 第三部分 **数据强化**
  ...
  这部分将名字单独定义了一下，主要考虑到这部分相关任务的重要性程度。尤其是在NLP领域。当前NLP普遍的问题是无标注数据（怎么提高标注效率，比如主动学习）、数据量少（使用数据增强）、数据标注不准确（标签校正），所以这部分重要性个人认为要比算法部分更加重要。
